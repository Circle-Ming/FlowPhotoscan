...
class returnCrossDomain(Thread):
    ...
    def matlab(self):
        os.system("python data_util.py")
        return "done"
    
    def rgb(self):
        os.system("python data_util2.py")
        with open('./middle/3.json', 'r') as f:
            data_ = json.load(f)
    ...
    def run(self):
        ...
        while True:
            if not self.isHandleShake:
                ...
                data['filename'] = str(clientData)
                if not os.path.exists('./middle'):
                    os.mkdir('./middle')

                if 'raw' in data['filename']:
                    with open('./middle/1.json', 'w', encoding='utf-8') as f:
                        json.dump(data, f)
                    result = self.matlab()
                elif 'png' in data['filename'] or 'jpg' in data['filename']:
                    with open('./middle/2.json', 'w', encoding='utf-8') as f:
                        json.dump(data, f)
                    result = self.rgb()
                # 处理数据
                ans = self.answer(clientData, result)
                self.sendDataToClient(ans)
                if (ans != "Unresolvable Command!" and ans != "hello world" and '.png' not in ans):
